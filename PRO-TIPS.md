...
- **NEVER ask the agent to just "do something."** ALWAYS ask the agent to plan first, review the plan, then execute after approval.
  ...
  - **My current workflow:**
    - First, turn incoming requirements into final specifications:
        - Whether I receive an externally authored feature spec or a brain dump, I use [generate-feature-spec](./skills/generate-feature-spec) to produce the initial specifications.
        - I then use [discuss-gray-areas](./skills/discuss-gray-areas) to resolve the remaining product and functional gray areas and generate the final specifications.
    - For focused small tasks:
        - I use [quick-task](./skills/qt) for small implementation work. It has the agent understand the task and ask clarifying questions, read code and context, plan properly, execute thoroughly, verify, and close cleanly. For bugs, I use [quick-debug](./skills/qd). Just replace your usual day-to-day tasks with `/qt <task description>` or `/qd <bug description>`.
    ...
    - For large or complex tasks
        - Start with the final specifications prepared above.
        - I first select plan mode in my agent (and select the most powerful model I have i.e. Opus 5 xhigh), use [deep-plan](./skills/deep-plan), and ask the agent to analyze, think hard, look at everything, make a good understanding, and make a perfect plan in `.planning/navoid-plans/<slug>/plan.md` with as much detail as possible and an executive summary. I review the executive summary, ask for adjustments if needed, and then approve it.
    ...
    - For extra large tasks (like 0 to 1 implementations)
        - I use the [GSD](https://github.com/open-gsd/gsd-core) framework. Very expensive and verbose, but effective.
...
- Always have a project AGENTS.md (or CLAUDE.md or whatever).
  ...
  - Always keep it updated; ask your agent to initialize (/init in Claude Code) it and update it after any major change. (i.e. have the instruction to update AGENTS.md at the end of it)
  - Also review and maintain it yourself sometimes. Remember, AGENTS.md is the general repo knowledge that YOU have, and that the agent should have as well to navigate properly in YOUR repo.
  ...
  - <details>
    <summary><b>My current guidelines at the end of AGENTS.md (after all the other info, under <code>## Very important AI Agent instructions</code>)</b></summary>
    ...
    - **when reporting information to me, be extremely concise, sacrifice grammar for concision.** Always write in ASD-STE100 to avoid ambiguity.
...
- **For repetitive tasks**, ALWAYS build custom skills and agents and/or plugins that employ skills to do them for you.
  ...
  - Examples: doing PR review, [making changes](./skills/qt), [fixing](./skills/qd), fixing deployments, etc.
...
- **Use sub-agents properly**
    - Sub-agents are great tools to save context window for the main agent, to do work in parallel, to delegate sub-tasks to weaker models, etc.
    - Learn how to use them and instruct your agent - sometimes even manually - to use sub-agents in a specific way to do your tasks.
    - Write your own custom sub-agents for specific tasks that you do repeatedly. (like task-verifier-agent before submitting your code for PR). Have your sessions use them when needed. (not real-life but educational example: `Diagnose security vulnerabilities with security-agent, then fix them all with multiple parallel vulnerability-patcher-agents all at once. Then verify with verifier agent, and do a final scan with security-agent to make sure nothing remains. Repeat this and ONLY stop when there are no security vulnerabilities left` and have somehting like this on a schedule to be executed every week)
- **Periodically clean up your code structure**

  - As you build with AI, the code might get messy - either architecturally, file structure, or code-wise. Ask your agent to evaluate what is important to you (architecture, tests, etc.) and generate a cleanup plan report. And pass to another agent to implement.
    - Real life example cleanup workflow: You've built an application with Temporal and have been creating various workflows and scheduled jobs. First, create a temporal-architect skill using the agent (ask it to do research and build it). Then ask your agent (new session) to lay out all your temporal workflows. Then ask it to read the temporal-architect skill and act as a temporal architect and review your temporal architecture and generate an analysis report. Then (new session) ask it to read the report and the skill, and generate a migration plan to the new improved architecture. Then (new session) ask it to implement the plan. Then (optional but recommended, also a new session) ask to verify the changes, AND create a manual testing guide for you to manually test the impacted features.
